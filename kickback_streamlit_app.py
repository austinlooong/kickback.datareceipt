...
def parse_youtube_watch_history(file):
    watched = []
    data = json.load(file)
    for item in data:
        if 'title' in item and item['title'].startswith("Watched "):
            watched.append(item['title'].replace("Watched ", "").strip())
    return watched, data

def generate_watch_summary(data, value_per_view=0.08):
    watched_titles, raw_data = parse_youtube_watch_history(data)
...
def parse_youtube_search_history(file, value_per_search=0.03):
    searches = []
    data = json.load(file)
    for item in data:
...
if watch_file and search_file:
    watch_summary = generate_watch_summary(watch_file)
    search_summary = parse_youtube_search_history(search_file)
