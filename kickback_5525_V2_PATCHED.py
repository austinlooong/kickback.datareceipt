...
def generate_data_label(watch_summary, search_summary):
    ...
    if 3 <= int(most_active_hour.split()[0]) <= 5:
...
def display_receipt(watch_summary, search_summary):
    ...
    if watch_summary:
        receipt_lines += [
            ...
            f"  Total Videos Watched:      {watch_summary['total_videos']}",
            f"  Most Active Hour:          {watch_summary['most_active_hour']}",
            f"  Estimated Value to Google: {watch_summary['estimated_value']}",
            ""
        ]
    ...
    if search_summary:
        receipt_lines += [
            ...
            f"  Total Searches:            {search_summary['total_searches']}",
            f"  Unique Terms:              {search_summary['unique_terms']}",
            f"  Estimated Value to Google: {search_summary['estimated_value']}",
            ""
        ]
    ...
    receipt_lines += [
        "💰 SUMMARY",
        f"  Total Value to Google:     ${total_value}",
        "  You Received:              $0.00 😐",
        "",
...
if zip_file:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if watch_path or search_path:
            ...
            st.error("Could not find either `watch-history.json` or `search-history.json` in your .zip file.")
